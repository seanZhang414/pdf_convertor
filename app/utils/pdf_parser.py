...
from app.utils.recognizer import Recognizer
from app.utils.layout_recognizer import LayoutRecognizer
from app.utils.table_structure_recognizer import TableStructureRecognizer
from app.utils.nlp import rag_tokenizer
...
class pdf_parser:
    ...
    def _updown_concat_features(self, up, down):
        ...
        LEN = 6
        tks_down = rag_tokenizer.tokenize(down["text"][:LEN]).split(" ") if down["text"] else []
        tks_up = rag_tokenizer.tokenize(up["text"][-LEN:]).split(" ") if up["text"] else []
        tks_all = (up["text"][-LEN:].strip() if up["text"] else "") \
                  + (" " if up["text"] and down["text"] and re.match(r"[a-zA-Z0-9]+",
                                                                     up["text"][-1] + down["text"][0]) else "") \
                  + (down["text"][:LEN].strip() if down["text"] else "")
        ...
        fea = [
            ...
            True if up["text"] and re.search(
                r"([。？！；!?;+)）]|[a-z]\.)$",
                up["text"]) else False,
            True if up["text"] and re.search(r"[，：‘“、0-9（+-]$", up["text"]) else False,
            True if down["text"] and re.search(
                r"(^.?[/,?;:\]，。；：’”？！》】）-])",
                down["text"]) else False,
            True if up["text"] and  re.match(r"[\(（][^\(\)（）]+[）\)]$", up["text"]) else False,
            True if up["text"] and  re.search(r"[，,][^。.]+$", up["text"]) else False,
            True if up["text"] and  re.search(r"[，,][^。.]+$", up["text"]) else False,
            True if up["text"] and  re.search(r"[\(（][^\)）]+$", up["text"])
                    and down["text"] and re.search(r"[\)）]", down["text"]) else False,
            ...
            True if down["text"] and  re.match(r"[0-9.%,-]+$", down["text"]) else False,
    ...
    def __images__(self, fnm, zoomin=3, page_from=0,
        ...
        try:
            ...
            self.page_chars = [
                [{**c, 'top': max(0, c['top'] - 10), 'bottom': max(0, c['bottom'] - 10)} for c in page.chars if
                 self._has_color(c)] for page in
